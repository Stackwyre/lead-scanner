...
def score_lead(title: str, text: str, published, topic_keywords: List[str], intent_keywords: List[str]) -> int:
    score = 0
    combined = f"{title} {text}".lower()
    
    # Strong intent in title = big points
    if any(w in title.lower() for w in ["looking for", "need help", "struggling with", "hiring", "seeking consultant"]):
        score += 6
    ...
    for sub in config["subreddits"]:
        try:
            ...
            for entry in feed.entries[:30]:  # latest 30 per sub
                ...
                published = datetime(*entry.published_parsed[:6], tzinfo=timezone.utc) if hasattr(entry, 'published_parsed') else datetime.now(timezone.utc)
                
                if not matches_intent(f"{title} {text}", config["intent_keywords"]):
                    continue
                
                score = score_lead(title, text, published, config["topic_keywords"], config["intent_keywords"])
...
def scan_x(config) -> List[Dict]:
    ...
    leads = []
    try:
        tweets = client.search_recent_tweets(query=query, max_results=15, tweet_fields=["created_at", "text", "public_metrics"])
        for tweet in tweets.data or []:
            text = tweet.text
            if not matches_intent(text, config["intent_keywords"]):
                continue
            score = score_lead(text, "", tweet.created_at, config["topic_keywords"], config["intent_keywords"])
            if score < 5:
                continue
            leads.append({
                "source": "x",
...
def generate_report(leads: List[Dict]) -> str:
    ...
    report += "**Harrison Eller** | Data Analytics • Industrial AI • Energy & Gas Turbines\n"
    report += "ASME Turbo Expo 2024 Author • 50+ turbines • 2.2% avg efficiency gains\n\n"
